...
class TrackStatisticCriteria extends Criteria
{
    ...
    public function onlyAccess(string $access): self
    {
        Assert::notEmpty($access, 'Access flag required.');
        Assert::inArray($access, self::ACCESS_STATUS, 'Access flag invalid. Use - all, assigned or unassigned.');

        return $this->cloneWithParam('access', $access);
    }
