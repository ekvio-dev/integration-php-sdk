 ...
 */
class Document implements MaterialDoc
{
    private const MATERIALS_DOC_ENDPOINT = '/v3/materials/document';
    /**
     * @var EqueoClient
     */
    private $client;
    ...
    public function create(array $criteria = []): array
    {
        ...
        if(array_key_exists('filename', $criteria)) {
            $fields['filename'] = $criteria['filename'];
        }

        $response = $this->client->request('POST', self::MATERIALS_DOC_ENDPOINT, $fields);
